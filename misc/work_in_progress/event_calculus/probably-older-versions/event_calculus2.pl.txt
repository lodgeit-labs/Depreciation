...
holdsIntervals(F,T1,T2,Duration):- T = T1, holdsAt(F,T), fd_dom(T1, D), dom_integers(D, Is), min_list(Is,T1), max_list(Is,T2), Duration is T2-T1.
holdsIntervals(F,T1,T2,Duration):- holdsExactlyBetween(F,T1,T2), Duration is T2-T1.
...
%holdsIntervals(checked(123),T1,T2,Duration).
%holdsIntervals(planning(123),T1,T2,Duration).
%findall((F,T1,T2),holdsIntervals(F,T1,T2,_),Results).
